...
void ots::IntensityInfoDQM::beginJob()
{
	...
	summary_histos->BookSummaryHistos(
	    tfs, "CAPHRI hits; nCAPHRIHits; Events", 100, 0, 100);
	// caloInfo
	summary_histos->BookSummaryHistos(
	    tfs, "IntensityInfo Calo, nHits; nCaloHits; Events/60", 200, 0, 12e3);
	summary_histos->BookSummaryHistos(
	    tfs, "IntensityInfo Calo, caloEnergy; E[MeV]; Events/(5 MeV)", 400, 0, 2e3);

	// tracker info
	summary_histos->BookSummaryHistos(
	    tfs, "IntensityInfo Tracker; nTrkHits", 200, 0, 12e3);
}
...
void ots::IntensityInfoDQM::analyze(art::Event const& event)
{
	...
	auto const caphriH =
	    event.getValidHandle<mu2e::CaloHitCollection>("CaloHitMakerFast::caphri");
	...
	{
		__COUT__ << "[IntensityInfoDQM::analyze] collecting summary histogram "
...
void ots::IntensityInfoDQM::beginRun(const art::Run& run)
{
	...
	__COUT__ << "[IntensityInfoDQM::beginRun] Run " << run.run()
